...
; ===========================================================================
; ---------------------------------------------------------------------------
; Unused Palette fading routine
; ---------------------------------------------------------------------------
		move.b	(byte_D4E7).w,d1
		bne.s	loc_750
...
loc_758:
		move.b	(byte_D4E7).w,(byte_D4E8).w
		bsr.s	sub_76E
		subq.b	#1,(byte_D4E9).w
		bne.s	locret_76C
		move.b	#0,(byte_D4E7).w
...
; Unused subroutine
; ---------------------------------------------------------------------------
		move.b	(byte_D4EC).w,d2
		bne.s	loc_7DC
		move.b	d0,(byte_D4EC).w
		move.b	d0,(byte_D4ED).w
		subq.b	#1,d1
		move.b	d1,(byte_D4EE).w
		move.l	a1,(lword_D4F0).w
		move.l	a2,(lword_D4F4).w

loc_7DC:
		subq.b	#1,(byte_D4ED).w
		bne.s	locret_7EC
		move.b	(byte_D4EC).w,(byte_D4ED).w
		bsr.w	sub_7EE
...
sub_7EE:
		movea.l	(lword_D4F0).w,a1
		movea.l	(lword_D4F4).w,a2
		moveq	#0,d6
		move.b	(byte_D4EE).w,d6
		moveq	#0,d7
...
.finish:
		...
		lsl.w	#2,d0
		lsl.w	#8,d0
		move.w	d0,(word_D816).w
		move.w	(a1)+,d0
		lsl.w	#2,d0
		lsl.w	#8,d0
		move.w	d0,(word_D81E).w
		move.w	(a1)+,d0
		lsl.w	#5,d0
		lsl.w	#8,d0
...
; ===========================================================================
; ---------------------------------------------------------------------------
; The Pattern Load Cue routines, unused due to the game not utilizing PLCs.
...
.submodes:
		bra.w	SegaAlt_Setup			; $00
; ---------------------------------------------------------------------------
		bra.w	loc_6CFC			; $04
; ---------------------------------------------------------------------------
		bra.w	SegaAlt_Scroll			; $08
; ---------------------------------------------------------------------------
		bra.w	loc_6CFC			; $0C
; ---------------------------------------------------------------------------
		bra.w	SegaAlt_SmallScroll		; $10
; ---------------------------------------------------------------------------
		bra.w	loc_6DDC			; $14
; ---------------------------------------------------------------------------
		bra.w	SegaAlt_ClumpTogether		; $18
; ---------------------------------------------------------------------------
		bra.w	SegaAlt_Shine			; $1C
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
		...
		bcc.s	loc_9B80
		move.w	#$10C,(word_C9FC).w
		bra.s	locret_9BDE
; ---------------------------------------------------------------------------

loc_9B80:
		move.w	#$114,(word_C9FC).w
		lea	(word_CA5E).w,a3
		move.w	(word_C9DE).w,d3
		neg.w	d3
		move.w	(word_CA1E).w,d4
		neg.w	d4
		moveq	#$1C-1,d7
		move.w	(word_CA1C).w,d2
		addq.w	#1,d2
		move.w	d2,(word_CA1C).w
...
loc_A3E0:
	...
	if FixBugs
		...
		andi.w	#8,d1
		eor.w	d1,$24(a6)
		tst.w	obj.Inertia(a6)
		bpl.s	loc_A42A
		neg.w	obj.Inertia(a6)
		bra.s	loc_A42A
; ---------------------------------------------------------------------------

loc_A40A:
		move.w	obj.Inertia(a6),d0
		bpl.s	loc_A412
		neg.w	d0
...
loc_B7DA:
		...
		adda.w	(a0,d0.w),a1
		adda.w	2(a0,d0.w),a2
		move.l	a1,obj.Map(a6)
		tst.b	6(a5)
		bne.s	loc_B80E
		move.l	a2,(lword_D888).w
		ori.b	#8,(byte_D87A).w
		rts
; ---------------------------------------------------------------------------

loc_B80E:
		move.l	a2,(lword_D88C).w
		ori.b	#$10,(byte_D87A).w
		rts
; ---------------------------------------------------------------------------
...
loc_BE2E:
		...
		tst.b	6(a5)
		bne.s	loc_BE66
		move.l	a2,(lword_D894).w
		ori.b	#$40,(byte_D87A).w
		rts
; ---------------------------------------------------------------------------

loc_BE66:
		move.l	a2,(lword_D898).w
		ori.b	#$80,(byte_D87A).w
		rts
...
loc_C2CE:
		...
		bgt.s	loc_C326
		beq.s	loc_C334
		sub.w	d1,obj.Ypos(a6)
		move.w	(sp)+,d2
		move.b	d2,obj.Angle(a6)
		move.l	(sp)+,d3
		moveq	#-1,d5
		rts
; ---------------------------------------------------------------------------
...
loc_CA8A:
		...
		bne.s	loc_CAB4
		add.w	d0,d3
		move.w	d3,obj.Xpos(a6)
		move.w	obj.Ypos(a5),d3
		btst	#4,d2
		bne.s	loc_CAAC
		add.w	d1,d3
		move.w	d3,obj.Ypos(a6)
		rts
; ---------------------------------------------------------------------------
...
loc_CC52:
		add.l	d3,obj.Xpos(a6)
		add.l	d4,obj.Ypos(a6)
		rts
...
loc_CE78:
		sub.b	obj.Angle(a1),d2
		cmpi.b	#$A0,d2
		bcs.w	loc_CED0
		cmpi.b	#$E0,d2
		bhi.w	loc_CED0
		move.b	#8,7(a1)
		move.b	obj.Angle(a1),d2
		btst	#3,$25(a1)
		beq.s	loc_CEA2
		addi.b	#$80,d2
...
loc_CED0:
		...
		btst	#3,$25(a1)
		beq.s	loc_CEE8
		sub.w	d0,obj.Inertia(a1)
		bra.s	loc_CEF2
; ---------------------------------------------------------------------------

loc_CEE8:
		add.w	d0,obj.Inertia(a1)
		bra.s	loc_CEF2
; ---------------------------------------------------------------------------
...
loc_CEF2:
		movea.w	(word_D854).w,a0
		movea.w	(word_D862).w,a1
		movea.w	(word_D864).w,a2
		move.l	8(a2),d1
		move.l	8(a1),d3
		sub.l	d1,d3
		asr.l	#3,d3
		move.l	obj.Ypos(a2),d2
		move.l	obj.Ypos(a1),d4
		sub.l	d2,d4
		asr.l	#3,d4
		btst	#0,(word_F000+1).w
		beq.s	loc_CF2A
		move.l	d3,d5
		move.l	d4,d6
		asr.l	#1,d5
...
sub_F116:
		bsr.w	sub_F238
		move.w	d2,(word_D8E8).w
		move.w	d3,(word_D8EA).w
		bsr.w	sub_F1CA
		bsr.w	sub_F136
		rts
...
loc_F140:
		...
		bne.s	loc_F1B8
		_move.w	0(a0),d0
		move.w	(word_D8DC).w,d1
		move.w	(word_D8E0).w,d2
		bsr.w	sub_F22C
		bcs.s	loc_F1B8
		move.w	2(a0),d0
		move.w	(word_D8DE).w,d1
		move.w	(word_D8E2).w,d2
		bsr.w	sub_F22C
		bcs.s	loc_F1B8
		movea.l	a1,a4
		lea	(unk_D8F2).w,a3
...
sub_F1CA:
		...
		addi.w	#$A0,d0
		addi.w	#$70,d1
		move.w	d0,(word_D8E4).w
		move.w	d1,(word_D8E6).w
		subi.w	#$140,d0
		bcc.s	loc_F1EE
		moveq	#0,d0

loc_F1EE:
		move.w	d0,(word_D8DC).w
		subi.w	#$E0,d1
		bcc.s	loc_F1FA
		moveq	#0,d1

loc_F1FA:
		move.w	d1,(word_D8DE).w
		move.w	(word_D8E4).w,d0
		move.w	(word_D8E6).w,d1
		addi.w	#$140,d0
		cmp.w	(word_D8E8).w,d0
		bcs.s	loc_F214
		move.w	(word_D8E8).w,d0

loc_F214:
		move.w	d0,(word_D8E0).w
		addi.w	#$E0,d1
		cmp.w	(word_D8EA).w,d1
		bcs.s	loc_F226
		move.w	(word_D8EA).w,d1

loc_F226:
		move.w	d1,(word_D8E2).w
		rts
...
sub_F390:
		tst.b	(unk_FDC1).w
		beq.s	locret_F3B4
		move.b	(byte_DA85).w,(byte_DA75).w
		subq.b	#2,(unk_FDC1).w
		tst.b	(unk_FDC1).w
		bgt.s	loc_F3AA
		bsr.w	Load_DMA_PLCs
...
unk_42358:
		dc.b $FF,$64,$FF,$70,$FF,$7C
unk_4235E:
		dc.b $FF,$82,$FF,$8E,$FF,$9A
unk_42364:
		dc.b $FF,$A0,$FF,$AC,$FF,$B8
		even
